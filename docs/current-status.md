...
| User/Case/Task memory isolation | P0 hardening implemented locally; deployment gates remain | Cognee results without explicit scope or valid expiry are rejected, new artifact registrations and retrieval require User/Case/Task ownership, and evidence re-ingest replaces stale source versions. Deployment still needs an inventory/quarantine pass for pre-existing unowned sidecars and live Cognee/worker verification. |
