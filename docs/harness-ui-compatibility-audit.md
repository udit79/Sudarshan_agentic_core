...
| Artifact Review / Approve / Reject | Compatible in the live bridge when an operator identity is configured | The bridge posts `/runs/{run_id}/resume` with the same `SUDARSHAN_OPERATOR_ID` identity used by governed ingestion (or the explicit `operatorId` bridge option); the API middleware therefore receives the required `X-Operator-Id`. Focused bridge and panel tests cover the authenticated review path. |
