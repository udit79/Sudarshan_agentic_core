...
- First `T17` staged-PPT slice: `SUDARSHAN_PPT_FLOW=staged` selects grounding, deck planning, visual routing, slide content, and quality tasks with named persisted callbacks; staged mode is now the default, legacy is an explicit comparison switch, and staged output is still converted to `PresentationOutput`.
