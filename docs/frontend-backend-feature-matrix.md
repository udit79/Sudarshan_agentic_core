...
| T47 | Open; benchmark gate | Benchmark runner/report archive for PDFs, PPTX, infographic, diagram, video, paraphrase, failure, and cross-skill cases | Benchmark dashboard with per-artifact thresholds, tokens, cost, latency, cache, repair, and human correction | Promotion is blocked on quality, security, or cost threshold failure |
...
| T63 | Open | Provider-neutral social read/write adapter, receipts, scopes, timeout, cancellation, audit | Connector setup/status and manual-mode state; never collect provider secrets in skill UI | No skill imports provider SDK or bypasses approval/scheduler |
| T64 | Open | LinkedIn provider/cache/timeout/retry/rate-limit configuration validation | Configuration status, missing-credential and draft fallback messaging | Startup validates shape without requiring credentials |
| T65 | Open | Bounded post/comment/thread reads, scoped versioned cache, provenance, untrusted-content marking | Read/source drawer with freshness, scope, provenance, invalidation state | Retrieved text cannot change tools, targets, approval, or publication |
| T66 | Open | Durable approval/resume/release, payload hash, actor, policy, idempotency, provider receipt | Approval inbox, diff, actor/policy display, approve/reject/resume states | No external write without an authorized approval event |
...
| T72 | Open | Social retry/rate-limit/idempotency state machine and provider receipts | Retry-after/countdown, pending, rate-limit, duplicate, and cancellation states | Provider failures never become successful-looking publication |
| T73 | Open | Durable schedule queue, timezone validation, cancellation, idempotent release | Calendar/scheduled item list, edit/cancel, pending/failed receipt | Same action cannot publish twice after restart |
| T74 | Open | Backend projections for LinkedIn workspace, approvals, reads, comments, schedules | Full LinkedIn workspace: composer, humanizer, comments/replies, visual child, approvals | Existing post projection remains compatible |
| T75 | Open | Optional thread-monitoring adapter with bounded polling and lifecycle policy | Opt-in monitoring view with pause/stop and privacy scope | Monitoring has explicit budget, retention, and cancellation |
| T76 | Open | Offline social provider test doubles and contract fixtures | Frontend integration tests run without provider credentials | Manual/draft mode and failure classes are testable offline |
...
| T80 | Open | Allow-listed infographic template/theme registry and versioned design tokens | Template/theme selector, preview, contrast/density warnings | Templates cannot inject arbitrary code or unsupported claims |
...
| T82 | Open | PPT targeted repair loop and repair patch contracts | Issue list, patch preview, apply/reject, slide rerender | Repair is targeted and evidence-preserving |
...
| PM-3–PM-9 | Open | Evidence workspace, DeckPlan renderer/template/dependency contracts, parallel slide jobs, child reconciliation, visual promotion, MCP/A2A/rollback gates | Evidence-to-slide workspace, template/theme editor, slide lanes, child artifacts, visual diff, approval and rollback | PPT Master is enabled only after evidence, SVG/native quality, and promotion gates pass |
...
| IF-4–IF-9 | Open | Allow-listed templates/themes, structure compilers, SVG/PNG exports, visual regression, renderer promotion, cache and interoperability | Streaming preview/editor, template/theme UI, export/download, quality diff, approval | No arbitrary syntax/JS/remote asset escapes the boundary |
...
| DD-3–DD-9 | Open | Semantic type registry, sanitized Mermaid/draw.io/Excalidraw adapters, style profiles, label/density regression, optional pinned motion | Type chooser, import wizard, profile/theme editor, HTML/SVG/PNG export, edit/preview, approval | Static remains default; imported or animated content is sanitized and auditable |
